...
// ProcessDemo parses a demo stream and returns a demoScrape2-compatible Game.
//
// Contract (mirrors demoScrape2 v0.3.2 for the demo worker classifier):
//   - The returned *Game is always non-nil.
//   - If the demo produced no valid rounds, ErrNoValidRounds is joined into the
//     returned error (so errors.Is(err, ErrNoValidRounds) holds even when the
//     parse also ended unexpectedly).
//   - Otherwise the raw ParseToEnd error (e.g. ErrUnexpectedEndOfDemo) is
...
func ProcessDemo(demo io.ReadCloser) (*Game, error) {
	...
	if len(game.Rounds) == 0 {
		// errors.Join drops nil, so this yields exactly ErrNoValidRounds when
		// ParseToEnd succeeded, or a joined error (still matching
		// errors.Is(err, ErrNoValidRounds)) when it did not.
		return game, errors.Join(err, ErrNoValidRounds)
	}
